...
from pca import remove_low_freq, remove_high_freq
...
@torch.no_grad()
def var_thresholded_l2(img, pred_img, mask, pca, mask_ratio, thresholds) -> dict:
    metrics = {}
    for th in thresholds:
        l2_bottom = ((remove_low_freq(img, pca, th) - remove_low_freq(pred_img, pca, th)).square() * mask).mean() / mask_ratio
        l2_top = ((remove_high_freq(img, pca, 1-th) - remove_high_freq(pred_img, pca, th)).square() * mask).mean() / mask_ratio
        
        metrics[f"val_l2/top_{th:.2f}"] = l2_top
        metrics[f"val_l2/bottom_{1-th:.2f}"] = l2_bottom
